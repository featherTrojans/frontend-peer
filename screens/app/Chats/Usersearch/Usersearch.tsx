...
const SingleUser = ({
  ...
  return (
    <TouchableOpacity
      activeOpacity={0.8}
      style={styles.userSearchContainer}
...
const Usersearch = ({ navigation, route }) => {
  ...
  const handleChangeText = (text) => {
    ...
    setSearchText(text);
    setFilteredContact(
      contactsResolved.filter(
        (contact) =>
          contact?.phoneNumber?.includes(text) ||
          contact?.username?.toLowerCase().includes(text) ||
      ...
      <View style={[styles.mainContainer]}>
        ...
        <View style={{ marginTop: 20, marginBottom: 28 }}>
          <TextInput
            ...
            onChangeText={
              active === "phonecontact" ? handleChangeText : handleFindUsername
            }
          />
        </View>
